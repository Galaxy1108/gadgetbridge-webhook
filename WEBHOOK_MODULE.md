...
**服务器地址（只填根地址）**：设置页填 `example.com` 即可，自动补全 `https://` 与
`/upload`（插件独立端点，无需令牌）；旧配置的完整 AstrBot 路由 URL（含 `/api/`）仍兼容。
上传被服务器判为 `pending_bind`（设备未配对）时，手机端进入"等待配对"状态：不推进
游标、不频繁重试，配对（/bind）成功后下一次周期上传自动恢复。
