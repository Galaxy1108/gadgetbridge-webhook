...
public class PreferenceMigrator63 extends AbstractPreferenceMigrator {
    private static final Logger LOG = LoggerFactory.getLogger(PreferenceMigrator63.class);

    private static final String PREF_CHARTS_TABS = "charts_tabs";
    private static final String CHART_TRAINING_READINESS = "trainingreadiness";
        ...
        try (DBHandler db = GBApplication.acquireDB()) {
            ...
            for (final Device dbDevice : activeDevices) {
                ...
                if (StringUtils.isBlank(chartsTabsValue)) {
                    newPrefValue = CHART_TRAINING_READINESS;
                } else if (chartsTabsValue.contains(CHART_TRAINING_READINESS)) {
                    newPrefValue = chartsTabsValue;
                } else {
                    newPrefValue = chartsTabsValue + "," + CHART_TRAINING_READINESS;
                }
            ...
            LOG.error("Failed to migrate prefs to version 63", e);
        }
    }
}
