...
public class PreferenceMigrator61 extends AbstractPreferenceMigrator {
    private static final Logger LOG = LoggerFactory.getLogger(PreferenceMigrator61.class);

    private static final String PREF_CHARTS_TABS = "charts_tabs";
    private static final String CHART_RACE_PREDICTION = "raceprediction";
        ...
        try (DBHandler db = GBApplication.acquireDB()) {
            ...
            for (final Device dbDevice : activeDevices) {
                ...
                if (StringUtils.isBlank(chartsTabsValue)) {
                    newPrefValue = CHART_RACE_PREDICTION;
                } else if (chartsTabsValue.contains(CHART_RACE_PREDICTION)) {
                    newPrefValue = chartsTabsValue;
                } else {
                    newPrefValue = chartsTabsValue + "," + CHART_RACE_PREDICTION;
                }
            ...
            LOG.error("Failed to migrate prefs to version 61", e);
        }
    }
}
